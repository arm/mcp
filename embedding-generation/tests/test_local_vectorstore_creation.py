...
def test_default_document_windows_are_lossless_and_non_overlapping():
    ...
    assert len(texts) > 1
    assert all(
        a["content_end_char"] == b["content_start_char"]
        for a, b in zip(metadata, metadata[1:])
    )
    assert all(row["embedding_window_overlap_tokens"] == 0 for row in metadata)
    assert all(row["embedding_window_policy"] == "lossless_windows" for row in metadata)


def test_positive_overlap_remains_bounded_and_measurable():
    ...
    assert_lossless(body, texts, metadata, PieceTokenizer(), 24)
    assert all(
        a["content_end_char"] > b["content_start_char"]
        for a, b in zip(metadata, metadata[1:])
    )
    assert all(row["embedding_window_overlap_tokens"] == 3 for row in metadata)


def test_window_ids_and_spans_are_deterministic():
    body = " ".join(f"deterministic{index}" for index in range(40))
    first = generator.prepare_embedding_records(
        [source(body)],
        PieceTokenizer(),
        max_seq_length=24,
    )
    second = generator.prepare_embedding_records(
        [source(body)],
        PieceTokenizer(),
        max_seq_length=24,
    )
    assert first == second
    metadata = first[1]
    assert [row["chunk_uuid"] for row in metadata] == [
        f"parent__window_{index}_of_{len(metadata)}"
        for index in range(1, len(metadata) + 1)
    ]


def test_every_window_repeats_title_and_heading_context():
    body = " ".join(f"word{index}" for index in range(50))
    texts, _ = generator.prepare_embedding_records(
        [source(body)],
        PieceTokenizer(),
        max_seq_length=24,
    )
    assert len(texts) > 1
    assert all(text.startswith("Manual\nInstall\n\n") for text in texts)


def test_subword_windows_prefer_whole_word_boundaries():
    body = " ".join(["abcdefgh"] * 12)
    spans = generator._window_spans(
        PieceTokenizer(), body, max_tokens=8, overlap_tokens=2
    )
    assert len(spans) > 1
    ...
    for start, end in spans:
        ...
        assert (
            len(
                PieceTokenizer()(body[start:end], add_special_tokens=False)["input_ids"]
            )
            <= 8
        )
...
def test_oversized_words_make_progress_without_losing_text():
    ...
    assert len(texts) > 1
    assert all(
        b["content_start_char"] > a["content_start_char"]
        for a, b in zip(metadata, metadata[1:])
    )
...
def test_seam_overflow_retries_smaller_budget_without_dropping_text():
    ...
    tokenizer = SeamTokenizer()
    with patch.object(
        generator, "_window_spans", wraps=generator._window_spans
    ) as spans:
        texts, metadata = generator.prepare_embedding_records(
            [source(body)],
            tokenizer,
...
def test_intrinsics_preserve_raw_input_and_one_vector():
    ...
    assert metadata[0]["embedding_was_truncated"] is True
    assert (
        metadata[0]["source_embedding_token_count"]
        > metadata[0]["embedding_token_count"]
    )
    assert metadata[0]["embedding_window_overlap_tokens"] == 0
