---
# Place this file at: .github/prompts/migrate-x86-to-arm.prompt.md
name: migrate-x86-to-arm
description: Migrate a codebase from x86 to Arm using MCP tools.
agent: agent
tools: ['check_image', 'skopeo', 'knowledge_base_search', 'migrate_ease_scan']
---
...
Steps to follow:
1. Look in all Dockerfiles and use #tool:check_image and/or #tool:skopeo to verify Arm compatibility, changing the base image if necessary.
2. Look at the packages installed by the Dockerfile and send each package to #tool:knowledge_base_search to check each package for Arm compatibility. If a package is not compatible, change it to a compatible version. When invoking the tool, explicitly ask "Is [package] compatible with Arm architecture?" where [package] is the name of the package.
3. Look at the contents of any requirements.txt files line-by-line and send each line to #tool:knowledge_base_search to check each package for Arm compatibility. If a package is not compatible, change it to a compatible version.
4. Look at the codebase that you have access to, and determine what language is used.
5. Run #tool:migrate_ease_scan on the codebase, using the appropriate language scanner based on what language the codebase uses, and apply the suggested changes.
...
- If you are unsure about Arm equivalents, use #tool:knowledge_base_search to find documentation.
